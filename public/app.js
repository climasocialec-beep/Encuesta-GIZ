...
function renderLogin() {
  ...
  document.getElementById('login-form').addEventListener('submit', event => {
    ...
    if (!user || document.getElementById('demo-pin').value !== 'demo') {
      showToast('Revisa tu usuario y clave de demostración');
      return;
    }
    currentUser = user;
...
async function loadRemoteState() {
  ...
  if (!currentCampaign) {
    ...
    if (contactWithCampaign?.campaign_id) currentCampaign = { id: contactWithCampaign.campaign_id, name: 'Campaña activa' };
  }
  try {
    const { data: outcomes } = await supabaseClient.from('outcomes').select('id, code');
